...
class PGMN_conv_block(conv_block_base):
    @dataclass
    class Options(conv_block_base.Options):
        ...
        #####   fzy    #####
        use_structural: bool = 0 # if 'False' structural component is not used
    ...
    def __init__(self, in_h, in_channels, out_channels, i_size, name=None, module_type='conv', initial_inv_block_lr=0.001,  deviation_threshold=3, freeze_module_after_step_limit=False, deeper=False, options:Options=Options(), num_classes: int=0, **kwargs):
        ...
        self.args: PGMN_conv_block.Options = copy.copy(options)
        ...
        #####   fzy   #####
        if not self.args.without_using_weighted_sums:
            self.weight_functional_component = nn.Parameter(data=torch.Tensor(1,out_channels),requires_grad=True)
    ...
    def update_stats_without_structural(self, m: torch.Tensor = torch.tensor(1.), threshold=0.5, record=False):
        '''
        Update the running stats of the module
        '''
        #######################
    ...
    def update_stats_copy(self, v: torch.Tensor, v_backup: torch.Tensor, m: torch.Tensor = torch.tensor(1.), threshold=0.5, record=False):
        '''
        Update the running stats of the module
        '''
        #######################
    ...
    def freeze_functional(self, inner_loop_free=True, if_eval_bn=False):
        if not self._frozen_functional_buffer:
            if not inner_loop_free:
                ...
                print(f'freezing classifier ')
                return True
            else:

                print(f'can not freeze functional {self.name}')
                return False
        else:            
...
class GatedOh(PGMN_conv_block):
    @dataclass                      
    class Options(PGMN_conv_block.Options):
        pass
